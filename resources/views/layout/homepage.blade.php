...
@section('content')
    ...
    <section id="home" class="bg-cover bg-center min-h-screen flex items-center justify-center" style="background-image: url('{{ asset('images/dummy.png') }}');">
        <div class="container mx-auto flex flex-col items-center justify-center text-center px-4">
            <div class="bg-white w-40 h-40 sm:w-60 sm:h-60 rounded-full flex flex-col items-center justify-center shadow-lg mb-4">
                <img src="{{ asset('images/logotext.png') }}" alt="Logo" class="w-28 h-28 sm:w-40 sm:h-40 object-contain">
            </div>
            <h1 class="text-biru font-bold text-2xl sm:text-4xl text-shadow-xl mb-2">Celebes Ocean Fisheries</h1>
            <p class="text-biru text-base sm:text-xl max-w-2xl mx-auto italic mb-1">Supplying Across the Globe</p>
            <p class="text-biru text-base sm:text-xl max-w-2xl mx-auto font-bold">SEAFOOD PROCESSOR EXPERT & EXPORTER</p>
        </div>
    </section>
    ...
    <section id="products" class="py-12 sm:py-20">
        <div class="container mx-auto text-center px-4">
            <h1 class="text-biru font-bold text-lg sm:text-xl mb-5 text-shadow-md">Products</h1>
            <p class="text-biru text-base sm:text-lg mb-5 max-w-3xl mx-auto">We provide fresh and frozen fishery products <br>
                All of our frozen and fresh fishery products are <b>caught fresh</b> from the ocean
            </p>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 m-5">
                @if ($products->isEmpty())
    ...
    <section id="about" class="py-10 px-4">
        <div class="flex items-center space-x-2 mb-4">
            ...
            <h2 class="text-base sm:text-lg md:text-xl font-bold text-biru whitespace-nowrap">Why Celebes Ocean Fisheries?</h2>
            <hr class="flex-grow border-t border-biru">
        </div>
        <div class="flex flex-col lg:flex-row items-center justify-center my-8 gap-6">
            <div class="lg:w-1/2 w-full mb-4 lg:mb-0 px-0 lg:px-12 text-justify text-biru">
                <p class="mt-4 text-center lg:text-justify">
            ...
            <div class="lg:w-1/2 w-full flex justify-center">
                <img src="{{ asset('images/pabrik.png') }}" alt="pabrik.png" class="w-full max-w-xs sm:max-w-sm md:max-w-md h-40 sm:h-60 lg:w-96 lg:h-60 rounded-md shadow-xl object-cover">
            </div>
        </div>
    </section>
    ...
    <section id="contact">
        <div class="max-w-4xl mx-auto px-4 my-10 py-10 rounded-xl">
            <h2 class="text-lg sm:text-xl font-bold text-biru">Be Our Partner in Your Country</h2>
            <p class="text-biru mb-6 italic">Please contact us to know more about our company, products, and sample requests</p>
            <form method="POST" enctype="multipart/form-data" action="{{ route('form.add') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-3 flex flex-col md:flex-row justify-end gap-2">
                    @guest
                    <button type="submit"
                        class="w-full md:w-auto bg-biru text-putihsusu font-semibold py-2 px-8 rounded-md hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors">
                        Submit
                    </button>
                    @endguest
                    @auth
